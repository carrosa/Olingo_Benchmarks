...
static void print_timing(uint64_t start, uint64_t end, const char *label) {
...
static void gen_message_1d(poly *Msg) // Accepts poly Msg[M] as a pointer
{
...
// static void dummy_poly_w(poly *p) {
...
static void benchmark() {
  ...
  start = get_cycles();
  as_sign_round3(&sig, *pks, ctx_r, ctx_s, ctx_z, Sei, mu, dsi[0], w_i, user,
                 users);
  end = get_cycles();
  ...
  start = get_cycles();
  as_sign_comb(&ctx_z, pks, &sig, pks->yprime, wprime, dsi);
  end = get_cycles();
