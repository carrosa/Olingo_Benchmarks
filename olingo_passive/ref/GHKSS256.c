...
void combine_1d(poly *Msg,     // Output: poly[L]  — accumulator of the result
...
// Safe, streaming version: no giant buf; fixed-width mpz_export with left
// padding
...
const mpz_srcptr QQ = GMP_q;
  ...
  if (!slot) {
    fprintf(stderr, "OOM in compute_challenge\n");
    abort();
  }

// Helper lambda-ish (C version): export coeff in [0,q) into fixed-width slot
#define EXPORT_COEFF_FIXED(gmpz_)                                              \
  do {                                                                         \
    /* ensure in [0,q) */                                                      \
...
#undef EXPORT_COEFF_FIXED
  ...
  // Finalize and squeeze 32 bytes (adjust if your poly_challenge expects a
  // different size)
  uint8_t hash[32];
                      ...
                      uint8_t h_yi[32]) // output hash
{
  ...
  if (!ei) {
    fprintf(stderr, "OOM in as_keygen_preenc(ei)\n");
    abort();
  }
  poly_1d_init(ei, K);
  for (int i = 0; i < K; i++) {
                ...
                poly **r_out)    // output: allocated r[M] (NTT domain)
{
  ...
  if (!r || !w) {
    fprintf(stderr, "alloc failed in as_sign_r1\n");
    exit(1);
  }
  poly_1d_init(r, M);
  poly_1d_init(w, K);
...
void as_sign_round3(sig_t *sig, pks_t pks, ctx_t ctx_r[THRESHOLD], ctx_t ctx_s,
                    ctx_t ctx_z, poly ski[LHAT][M],
                    poly mu[M],  // L, but padded to M
                    poly dsi[M], // L, but padded to M
                    poly w_i[THRESHOLD][K], int user,
                    int32_t users[USERS]) {
  // Compute w = sum(w_i) for all i \in U
  // Compute rounded w, round by q_w
  poly w[K];
              ...
              int32_t *users)   // users[USERS]
{
  ...
  // Since benchmarking not needed for other than computation, need to store and
