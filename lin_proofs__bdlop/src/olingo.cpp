...
static void lin_hash(params::poly_q &beta, comkey_t &key, commit_t x,
                     commit_t y[SIZE], params::poly_q alpha[SIZE],
                     params::poly_q u[SIZE], params::poly_q t,
                     params::poly_q _t[SIZE]) {
...
static int rej_sampling_ptrs(const vector<const params::poly_q *> &zptrs,
  ...
  double r,
      M = exp((T * 1.14 * sqrt(0.667) * NONZERO * sqrt(WIDTH * N)) / (2.0 * s2));
  mpf_t u;
  uint8_t buf[8];
  int64_t seed;
  ...
  for (size_t idx = 0; idx < zptrs.size(); ++idx) {
    ...
    for (size_t k = 0; k < params::poly_q::degree; ++k) {
      util::center(coeffs0[k], coeffs0[k], params::poly_q::moduli_product(),
                   qDivBy2);
      util::center(coeffs1[k], coeffs1[k], params::poly_q::moduli_product(),
                   qDivBy2);
...
// New
static void lin_prover(params::poly_q y_E[WIDTH],
                       params::poly_q y_S[SIZE][WIDTH], params::poly_q &t_E,
                       params::poly_q t_S[SIZE], params::poly_q u[SIZE],
                       commit_t com_E, commit_t com_S[SIZE],
                       params::poly_q alpha[SIZE], comkey_t &key,
                       vector<params::poly_q> r_E,
  ...
  do {
    ...
    /* Sample challenge. */
    lin_hash(chal, key, com_E, com_S, alpha, u, t_E, t_S);
    ...
    rej1 = rej_sampling_ptrs(zptrs, vptrs,
                             (uint64_t)sigma_c * (uint64_t)sigma_c, SIZE + 1);
  } while (rej1);
...
// New
static int lin_verifier(params::poly_q z_E[WIDTH],
                        params::poly_q z_S[SIZE][WIDTH], params::poly_q t_E,
                        params::poly_q t_S[SIZE], params::poly_q u[SIZE],
                        commit_t com_E, commit_t com_S[SIZE],
                        params::poly_q alpha[SIZE], comkey_t &key,
...
inline params::poly_q make_scale_poly(uint64_t lambda = 1) {
  const char *q_str = "23473657271235664258052971185590273";
...
inline void mul_A2_y(vector<params::poly_q> &result,
...
static void bench_lin() {
  ...
  commit_t com_E;
  commit_t com_S[SIZE];
  params::poly_q alpha[SIZE], u[SIZE], t_E, t_S[SIZE], y_E[WIDTH],
      y_S[SIZE][WIDTH];
  vector<params::poly_q> r_E(WIDTH), s(WIDTH); // Randomness vectors.
  vector<vector<params::poly_q>> r_S(
      SIZE,
      vector<params::poly_q>(WIDTH)); // Randomness vectors for secret matrix.
...
//   int ver = lin_verifier(y_E, y_S, t_E, t_S, u, com_E, com_S, alpha, key, beta);
  ...
  BENCH_BEGIN("linear verifier") {
    BENCH_ADD(
        lin_verifier(y_E, y_S, t_E, t_S, u, com_E, com_S, alpha, key, beta));
  }
  BENCH_END;
}
...
static inline void hash_poly_bytes(blake3_hasher *h, const params::poly_q &p) {
  ...
  blake3_hasher_update(h, reinterpret_cast<const uint8_t *>(ptr),
                       BYTES_PER_POLY);
}
static void lin_hash_dkg(params::poly_q &chal, comkey_t &key, commit_t &x,
                         vector<commit_t> &y, vector<params::poly_q> &alpha,
                         vector<params::poly_q> &u, params::poly_q &t,
...
static void lin_hash_dkg2(params::poly_q &chal, comkey_t &key,
                          vector<commit_t> &y,
...
static void lin_prover_dkg(
    vector<params::poly_q> &y_S, vector<vector<params::poly_q>> &y_Si,
    params::poly_q &t_S, vector<params::poly_q> &t_Si,
    vector<params::poly_q> &u, commit_t &com_S, vector<commit_t> &com_Si,
    vector<params::poly_q> &lambdai, comkey_t &key, vector<params::poly_q> &r_S,
  ...
  mpz_fdiv_q_2exp(qDivBy2, params::poly_q::moduli_product(), 1);
  double sigma_c =
      0.675 * (THRESHOLD + 1) * 1.14 * sqrt(0.667) * NONZERO * sqrt(WIDTH * N);
  ...
  do {
    ...
    rej1 = rej_sampling_ptrs(
        zptrs, vptrs, (uint64_t)sigma_c * (uint64_t)sigma_c, THRESHOLD + 1);
  } while (rej0 || rej1);
  mpz_clear(qDivBy2);
  for (size_t i = 0; i < params::poly_q::degree; i++) {
    mpz_clear(coeffs[i]);
...
static int lin_verifier_dkg(vector<params::poly_q> &z_S,
                            vector<vector<params::poly_q>> &z_Si,
                            params::poly_q &t_S, vector<params::poly_q> &t_Si,
                            vector<params::poly_q> &u, commit_t &com_S,
                            vector<commit_t> &com_Si,
                            vector<params::poly_q> &lambdai, comkey_t &key,
  ...
  double sigma_c =
      0.675 * THRESHOLD * 1.14 * sqrt(0.667) * NONZERO * sqrt(WIDTH * N);
  lin_hash_dkg(chal, key, com_S, com_Si, lambdai, u, t_S, t_Si);
...
void bench_lin_dkg3(int THRESHOLD) {
  ...
  vector<vector<params::poly_q>> Si(
      THRESHOLD,
      vector<params::poly_q>(SIZE)); // Secret shares (1 columns of shares)
  ...
  BENCH_BEGIN("linear proof"){
      // BENCH_ADD(lin_prover(y_E, y_S, t_E, t_S, u, com_E, com_S, alpha, key,
      // r_E, r_S));
      BENCH_ADD(lin_prover_dkg(y_S, y_Si, t_S, t_Si, u, com_S, com_Si, lambdai,
                               key, r_S, r_Si, THRESHOLD))} BENCH_END;
  BENCH_BEGIN("verify") {
    BENCH_ADD(lin_verifier_dkg(y_S, y_Si, t_S, t_Si, u, com_S, com_Si, lambdai,
                               key, S, THRESHOLD));
  }
  BENCH_END;
}
...
static void lin_prover_dkg2(vector<vector<params::poly_q>> &y_Si,
                            vector<params::poly_q> &t_Si,
                            vector<params::poly_q> &u, vector<commit_t> &com_Si,
                            comkey_t &key, vector<vector<params::poly_q>> &r_Si,
                            vector<params::poly_q> &codewordi, int FULL) {
  ...
  double sigma_c = 0.675 * FULL * 1.14 * sqrt(0.667) * NONZERO * sqrt(WIDTH * N);
  ...
  do {
    ...
    rej1 = rej_sampling_ptrs(zptrs, vptrs,
                             (uint64_t)sigma_c * (uint64_t)sigma_c, FULL);
  } while (rej1);
  mpz_clear(qDivBy2);
  for (size_t i = 0; i < params::poly_q::degree; i++) {
    mpz_clear(coeffs[i]);
...
static int
lin_verifier_dkg2(vector<vector<params::poly_q>> &z_Si,
                  vector<params::poly_q> &t_Si, vector<params::poly_q> &u,
                  vector<commit_t> &com_Si, vector<params::poly_q> &codewordi,
                  vector<params::poly_q> &W, comkey_t &key, int THRESHOLD) {
  int result = 1;
  params::poly_q chal, v, tmp, zero = 0;
  vector<params::poly_q> _v(THRESHOLD);
  // vector<params::poly_q> lhs(WIDTH), rhs(WIDTH);
...
void bench_lin_dkg2(int FULL) {
  ...
  // Sample codewordi
  for (int i = 0; i < FULL; i++) {
    // Should in actuality sample codewords here, not uniform constant
    // polynomials
    codewordi[i] = sample_const_poly_from_uniform();
  }

  // Compute dummy W used for benchmarking only (not in real protocol, but gives
  ...
  BENCH_BEGIN("linear proof") {
    BENCH_ADD(
        lin_prover_dkg2(y_Si, t_Si, u, com_Si, key, r_Si, codewordi, FULL));
  }
  BENCH_END;
  BENCH_BEGIN("linear verifier") {
    BENCH_ADD(
        lin_verifier_dkg2(y_Si, t_Si, u, com_Si, codewordi, W, key, FULL));
  }
  BENCH_END;
}
